...
for i=1:length(outcomes)
    ...
    ci1 = 1.96 * nanstd(var1) / sqrt(length(var1));
    ci2 = 1.96 * nanstd(var2) / sqrt(length(var2));
    ci3 = 1.96 * nanstd(var3) / sqrt(length(var3));

    [mdl1, mdl2] = test_group_diffs_at_5yr_nobl(outcome_name, fiveyrAttr);
    [smd1, smd2] = smd_at_5yr_nobl(outcome_name, fiveyrAttr);
    
    fprintf(['%s\t%3.2f (%3.2f, %3.2f)\t%3.2f (%3.2f, %3.2f)\t%3.2f (%3.2f, %3.2f)\t' ...
                '%3.2f\t%0.3f\t%3.2f\t%0.3f\t%3.2f\t%0.3f\n'], ...
            outcome_name, nanmean(var1), nanmean(var1)-ci1, nanmean(var1)+ci1, nanmean(var2), nanmean(var2)-ci2, nanmean(var2)+ci2, nanmean(var3), nanmean(var3)-ci3, nanmean(var3)+ci3, ...
