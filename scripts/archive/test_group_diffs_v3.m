...
for i = 3:15
    ...
    if i < 30
        ...
        adjustedMeans = adjustedMeans.AdjustedMean;
    
        fprintf(['%s\t%3.2f\t%3.2f\t%3.2f\t' ...
            '%3.2f\t%0.3f\t%3.2f\t%3.2f\t%0.3f\t%3.2f\n'], ...
        outcome_name, adjustedMeans(1), adjustedMeans(2), adjustedMeans(3), ...
...
baselineData = data(data.time == -1, {'id' outcome_name 'group', 'time' 'age' 'gender'});
postData = data(data.time == 1, {'id' outcome_name 'group', 'time' 'age' 'gender'});
...
mergedData.Properties.VariableNames{5} = 'age';
mergedData.Properties.VariableNames{6} = 'gender';
mergedData.Properties.VariableNames{7} = 'Baseline';
